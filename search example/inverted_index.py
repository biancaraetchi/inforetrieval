...
class InvertedIndex:
  ...
  #             [a, b, c, d] = a - document identifier
  #                            b - term frequency of term x in document a 
  #                            c - the tf-idf value if calculated,
  ...
  def __init__(self):
    ...
    # Create global stemmer for efficiency reasons (instantiate once)
    self.stemmer = PorterStemmer()
    
    # Local variable for holding the term by document matrix for doing 
    # calculations.  This will have to be created "on demand"
    self.A = None
  ...
  def add_document(self, document):
    ...
    # Returns collection dictionary Counter({'term1' : x_1, 'term2' : x_2, ...})
    # This is unordered (dictionary), but easy to traverse/update
  ...
  # index.  Just traverses the terms list for a match.  Left this in here
  # if you wanted to see the difference in speed between updating an inverted
  # index with linear search as opposed to binary search of terms.
  #
  # @param t - Term to search for.
  #
  # @return w - Term data if found (list)
...
####
  ...
  #
  # @return A - Term by document matrix (with TF or TFIDF/LogEntropy values) from 
